...
speak_key "admin.services_restart"

chmod +x \
    "${SCRIPT_DIR}/restart_websocket.sh" \
    "${SCRIPT_DIR}/restart_web_server.sh" \
    "${SCRIPT_DIR}/restart_python.sh" \
    "${SCRIPT_DIR}/reboot_robot.sh" \
    "${SCRIPT_DIR}/shutdown_robot.sh" 2>/dev/null || true
